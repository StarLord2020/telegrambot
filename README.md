...
- Способ B (через Telegram file_id):
  ...
  Примечание: для записей с `file_id` обязательно указывайте `tg_type`:
  - `tg_type: "audio"` — если файл был отправлен боту как «Музыка/Audio».
  - `tg_type: "voice"` — если это голосовое.
  - `tg_type: "document"` — если отправляли как документ.
  Без `tg_type` элемент будет пропущен в inline‑выдаче, чтобы не ломать весь список.
  ...
  Если вы отправляли файл как «Документ», используйте `document`:
  ```json
  {
    "id": "doc-1",
    "title": "Мой файл (mp3)",
  ...
  ```
  В inline‑выдаче такой элемент вернётся как документ. Чтобы играть «как музыка», отправляйте файл боту именно как «Музыка/Audio», а не как «Документ».

### Если пропали все варианты
- Один некорректный элемент может «сломать» inline‑ответ. Убедитесь, что для `file_id` указан верный `tg_type` (audio/voice/document) или есть валидный `url`.
- Для `voice` и `document` в inline требуется `title`.
